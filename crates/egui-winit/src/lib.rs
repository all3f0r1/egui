...
impl State {
    ...
    pub fn handle_platform_output(
        ...
        self.handle_platform_output_with_event_loop(window, None, platform_output);
    }

    /// Same as [`Self::handle_platform_output`] but threads the
    /// `ActiveEventLoop` so we can register a `winit::CustomCursor` from
    /// `PlatformOutput::cursor_image`. Pass `None` to silently drop any
    /// custom cursor request — the standard `cursor_icon` path still
    /// runs, which is the right fallback for integration paths that
    /// don't have access to the event loop (e.g. immediate viewports).
